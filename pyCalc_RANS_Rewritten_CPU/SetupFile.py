...
def Setup(fileX, fileY):
    ...
    # Apply data to GeometricData file
    GData.x2d = x2d
    GData.y2d = y2d
    GData.xp2d = xp2d
    GData.yp2d = yp2d
    ...
    dist_n = ywall_n - yp2d
    GData.wall_dist = np.minimum(dist_s, dist_n)

    #  west face coordinate
    GData.xw = 0.5*(x2d[0: -1, 0: -1] + x2d[0: -1, 1:])
    GData.yw = 0.5*(y2d[0: -1, 0: -1] + y2d[0: -1, 1:])

    # Interpolation factors (x)
    del1x = ((GData.xw - xp2d)**2 + (GData.yw - yp2d)**2)**0.5
    del2x = ((GData.xw - np.roll(xp2d, 1, axis=0))**2 +
             (GData.yw - np.roll(yp2d, 1, axis=0))**2)**0.5
    GData.fx = del2x / (del1x + del2x)

    #  south face coordinate
    GData.xs = 0.5*(x2d[0: -1, 0: -1] + x2d[1:, 0: -1])
    GData.ys = 0.5*(y2d[0: -1, 0: -1] + y2d[1:, 0: -1])

    # Interpolation factors (y)
    del1y = ((GData.xs - xp2d)**2 + (GData.ys - yp2d)**2)**0.5
    del2y = ((GData.xs - np.roll(xp2d, 1, axis=1))**2 +
             (GData.ys - np.roll(yp2d, 1, axis=1))**2)**0.5
    ...
    areaz_1 = 0.5*np.absolute(ax[0: -1, :] *
                              by[:, 0: -1] - ay[0: -1, :]*bx[:, 0: -1])
    ...
    # Initialize (pre-allocate) some variables
    Init()
    ...
    FData.pBC_e, FData.pBC_w,FData. pBC_n, FData.pBC_s, FData.pBC_Types,
    FData.kBC_e, FData.kBC_w, FData.kBC_n, FData.kBC_s, FData.kBC_Types,
    FData.omegaBC_e, FData.omegaBC_w, FData.omegaBC_n, FData.omegaBC_s, FData.omegaBC_Types) = GetBC(GData.ni, GData.nj)
    ...
    FData.conv_w, FData.conv_s = Convection(ap2d_vel, FData.u2d, FData.v2d, FData.p2d, FData.uface_w,FData. uface_s, FData.vface_w, FData.vface_s, FData.uBC_e, FData.uBC_w, FData.uBC_n, FData.uBC_s, FData.uBC_Types,\
                                            FData.vBC_e, FData.vBC_w, FData.vBC_n, FData.vBC_s, FData.vBC_Types, \
    ...
    if consts.k_omega: 
       urf_temp = consts.urfvis # no under-relaxation
       urfvis = 1
       FData.vis2d = vist_kom(FData.vis2d, FData.k2d, FData.omega2d)
       urfvis = urf_temp
   

   #############     CONSTANT DATA     ##################

    consts.i_monitor = ni-10
    consts.j_monitor = int(nj/2)
